"""
Skip-gram (Word2Vec) Embedding Module
...
class SkipGramEmbedding:
    """
    Skip-gram Word2Vec embedding class that can be saved and loaded
    ...
    def fit(self, tokenized_texts):
        ...
        self.is_fitted = True
        print(f"Skip-gram model trained. Vocabulary size: {len(self.model.wv)}")
    ...
    def save(self, filepath):
        ...
        Args:
            filepath: Path to save the model (e.g., 'embeddings/skipgram_model.bin')
        """
        if not self.is_fitted:
        ...
        print(f"Skip-gram model saved to {filepath}")
    
    def load(self, filepath):
        """
        Load a saved Skip-gram model
        ...
        self.is_fitted = True
        print(f"Skip-gram model loaded from {filepath}")
